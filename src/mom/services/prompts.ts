...
- Semantic search the codebase (searchCodebase)
...
Current working directory: ${process.cwd()}`;
    ...
    const extensionToolsPrompt = renderExtensionTools(mode);
    if (extensionToolsPrompt) {
...
function renderExtensionTools(mode: AgentMode): string {
    const enabled = extensionRunner.getEnabledExtensions();
    if (enabled.length === 0) return "";

    const toolEntries: string[] = [];
    for (const ext of enabled) {
        for (const [name, toolDef] of ext.tools) {
            const modes = toolDef.modes ?? ["build"];
            if (!modes.includes(mode)) continue;
            toolEntries.push(
                "  <tool name=\"" + name + "\" extension=\"" + ext.name + "\">" + toolDef.description + "</tool>",
            );
        }
    }
    ...
    return "# Extension Tools\n\nThe following additional tools are provided by loaded extensions:\n\n<extension_tools>\n" + toolEntries.join("\n") + "\n</extension_tools>";
}
