...
export interface ExtensionsConfig {
    /** Additional extension paths (files or directories) */
    paths: string[];
    /** Extension names that are explicitly disabled */
    disabled: string[];
    /** Bundled extension names that the user has explicitly enabled */
...
class ConfigService {
        ...
        try {
            ...
            if (rawConfig.apiKey && !rawConfig.providers) {
                const migratedConfig: ZencodeConfig = {
                    ...
                    extensions: rawConfig.extensions || DEFAULT_CONFIG.extensions,
                };
    ...
    /**
     * Get API key for a provider (supports both builtin and extension providers)
     */
    ...
    /**
     * Set API key for a provider (supports both builtin and extension providers)
     */
    ...
    getExtensionPaths(): string[] {
        return this.load().extensions?.paths ?? [];
    }

    /**
     * Get disabled extension names
     */
    getDisabledExtensions(): string[] {
        return this.load().extensions?.disabled ?? [];
    }

    /**
     * Add an extension name to the disabled list (persists across restarts).
     */
    addDisabledExtension(name: string): void {
        const config = this.load();
        if (!config.extensions) {
            config.extensions = { paths: [], disabled: [], enabledBundled: [] };
        }
        if (!config.extensions.disabled.includes(name)) {
            config.extensions.disabled.push(name);
            this.save(config);
        }
    }

    /**
     * Remove an extension name from the disabled list (re-enables it).
     */
    removeDisabledExtension(name: string): void {
        const config = this.load();
        if (!config.extensions?.disabled) return;
        config.extensions.disabled = config.extensions.disabled.filter(
            (n) => n !== name,
        );
        this.save(config);
    }
    ...
    addExtensionPath(extensionPath: string): void {
        ...
        if (!config.extensions.paths.includes(extensionPath)) {
            config.extensions.paths.push(extensionPath);
            this.save(config);
        }
    }

    /**
     * Remove an extension path
     */
    removeExtensionPath(extensionPath: string): void {
        const config = this.load();
        if (!config.extensions?.paths) return;
        config.extensions.paths = config.extensions.paths.filter(
            (p) => p !== extensionPath,
        );
        this.save(config);
    }
    ...
    /**
     * Get the list of bundled extension names the user has enabled.
     */
    getEnabledBundled(): string[] {
        return this.load().extensions?.enabledBundled ?? [];
    }

    /**
     * Mark a bundled extension as enabled by the user.
     */
    enableBundledExtension(name: string): void {
        const config = this.load();
        if (!config.extensions) {
            config.extensions = { paths: [], disabled: [], enabledBundled: [] };
        }
        if (!config.extensions.enabledBundled) {
            config.extensions.enabledBundled = [];
        }
        if (!config.extensions.enabledBundled.includes(name)) {
            config.extensions.enabledBundled.push(name);
            this.save(config);
        }
    }

    /**
     * Remove a bundled extension from the enabled list (back to disabled default).
     */
    disableBundledExtension(name: string): void {
        const config = this.load();
        if (!config.extensions?.enabledBundled) return;
        config.extensions.enabledBundled =
            config.extensions.enabledBundled.filter((n) => n !== name);
        this.save(config);
    }
}
