...
import { reindexSingleFile } from "../embeddings/indexer";
...
async function doWriteFile(
    ...
    return withToolScheduling("write", async () => {
        try {
            ...
            const relPath = path.relative(projectDir, resolvedPath);
            reindexSingleFile(projectDir, relPath, normalizedContent).catch(
                () => {},
            );
...
export const writeFile = tool({
    ...
    execute: async function* ({ path: filePath, content }) {
        for (
            let i = STREAM_CHUNK_SIZE;
            i < content.length;
            i += STREAM_CHUNK_SIZE
        ) {
            yield { streaming: true, content: content.slice(0, i) };
            await new Promise((r) => setTimeout(r, STREAM_DELAY_MS));
        }
        yield { streaming: true, content };

        const result = await doWriteFile(filePath, content);
        yield result;
    },
});
