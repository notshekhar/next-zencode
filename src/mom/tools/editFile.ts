...
async function doEditFile(
    ...
    return withToolScheduling("write", async () => {
        try {
            ...
            reindexSingleFile(projectDir, relPath, newContent).catch(
                () => {},
            );
...
export const editFile = tool({
    ...
    execute: async function* ({
        ...
        for (
            ...
            yield { streaming: true, content: newText.slice(0, i) };
            await new Promise((r) => setTimeout(r, STREAM_DELAY_MS));
        }
        yield { streaming: true, content: newText };

        const result = await doEditFile(
            filePath,
            oldText,
            newText,
            replaceAll,
